...
The firmware in this repository is a bench and diagnostic tool for the v0 node: a Seeed Studio XIAO ESP32-S3 with a Wio-SX1262 LoRa board and a RockBLOCK 9603 Iridium SBD modem. It tests the modem link, passes typed AT commands through to the modem, and offers the same console over USB and Bluetooth. It is not the node's product firmware; that is [meshsat-firmware](https://github.com/meshsat/meshsat-firmware).

Verified on 19 Sep 2026 with the XIAO on a power bank and the console over Bluetooth:
- `AT` → `"AT\r\r\nOK\r\n"` in 9 ms, including the automatic test right after a XIAO reset.
- All read-only AT commands answered in pass-through: `AT+CGMR` shows a 9603N rev DE with firmware TA21004, `AT&V` shows flow control off in the stored profile, and `AT+IPR?` shows 19200.
- One Iridium SBD session in pass-through: a mobile-originated message delivered, and a mobile-terminated message received in the same session.

Sources, checked pin numbers, and the places where official documents disagree: [REFERENCES.md](REFERENCES.md).
...
**Do not connect OnOff (pin 7) to a XIAO GPIO.** The RockBLOCK 9603 Rev F reads OnOff against its supply voltage: ON needs at least supply − 0.5 V (about 4.5 V on 5 V) or a floating pin, and more than 1 µA of leakage can switch the modem off. A 3.3 V pin can do neither. Switching the modem from firmware needs an open-drain (N-MOSFET) stage.

The SX1262 uses GPIO7, 8, 9, 38, 39, 40, 41 and 42. The Wio board's button and LED use GPIO21 and 48. None of these are D6/D7. The bench firmware does not configure or drive any of the radio pins.
...
**With the XIAO on a computer for the console:**

- XIAO USB-C → computer (console and flashing).
- RockBLOCK pin 8 → a separate 5 V source rated ≥ 500 mA, for example a power bank through a USB breakout.
- RockBLOCK pin 10 → that source's ground **and** a XIAO GND pin (common ground).

Do not feed a power bank's 5 V into the XIAO's **5V pin** while the XIAO is plugged into a computer. That pin is the USB VBUS line with no diode (XIAO schematic v1.4), so it would back-feed the computer's USB port. A computer port (500 mA on USB 2.0) is also too weak to power the RockBLOCK through the XIAO.

**On a power bank only:**
...
A power bank's **USB-A** port can switch itself off because the XIAO draws so little. An Anker Prime 20,000 mAh did exactly that on the bench, and the XIAO stopped advertising over Bluetooth. Anker's [trickle-charging mode](https://service.anker.com/article-description/What-is-Trickle-Charging-Mode) keeps USB-A on but limits the output to under 0.5 A: fine for the XIAO alone, too little for XIAO plus RockBLOCK.
...
On failure the log gives the likely causes in order: wiring, power, the 2-second power cycle, flow control. Any line ending you type (CR, LF or CR LF) reaches the modem as a single CR, because the 9603 accepts only CR.

**Iridium sessions cost money.** In pass-through, `AT+SBDIX` and anything else that opens a satellite session is billed by your airtime provider, even with an empty outbound buffer. Read-only commands such as `AT+CGMR` or `AT+CSQ` are free.

### Over Bluetooth

The same console is available over Bluetooth LE, so the XIAO can run from a power bank with no USB cable. It uses the Nordic UART Service and advertises as `meshsat-esp32-XXXX`, where XXXX is the end of the Bluetooth MAC. Any NUS client works: nRF Connect, a "Serial Bluetooth Terminal" app, or `tools/ble_console.py` (needs `pip install bleak`).
...
This is a bench console. The link is not encrypted, so the PIN travels in the clear, and once unlocked, pass-through can send any AT command to the modem, including ones that start billable Iridium sessions. Don't leave a node advertising in a public place with a PIN others could have seen. The product firmware protects its Iridium service with Bluetooth pairing instead.
...
- [x] `AT` → `OK` over GPIO43/44 with the wiring above. Passed 19 Sep 2026 in 9 ms.
...
src/diag/         AT link test and pass-through
...
- The Iridium driver knows nothing about Bluetooth, Wi-Fi or routing.
